 ...
 */
  ...
  _mouseClick(event) {
    ...
    if (event.button === 0 && this.inputSenderChannel && this.inputSenderChannel.readyState === 'open') {
      const rect = this.videoElement.getBoundingClientRect();
      const x = (event.clientX - rect.left) / rect.width;
      const y = (event.clientY - rect.top) / rect.height;
      console.log(`Left click: x=${x.toFixed(4)}, y=${y.toFixed(4)}`);
      const msg = JSON.stringify({ type: 'click', x: x, y: y });
